...
public class FusionRestaurant implements Restaurant {
    ...
    // @Qualifier를 필드주입에서 사용하는 경우는 @Autowired 다음줄에 @Qualifier를 작성하면 된다.
    private Chef chef;
    // 요리코스
   // @Autowired
    ...
    //@Autowired
    // 주입받을 파라미터 왼쪽에 @Qualifier("빈이름") 을 입력시, 지정 주입이 가능하다.
    public FusionRestaurant(@Qualifier("cc") Chef chef, @Qualifier("cco") Course course){
        this.chef = chef;
        this.course = course;
    }

    // Setter 주입시 @Qualifier를 사용하는 경우, 디폴트 생성자가 오버로딩 되어있어야 한다.
    public FusionRestaurant() {} // 리턴자료형 없는 디폴트 생성자
